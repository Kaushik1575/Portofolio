    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between h-20">
          <motion.a
            href="#"
            className="text-2xl font-bold bg-gradient-to-r from-gray-50 to-purple-500 bg-clip-text text-transparent hover:tracking-wider transition-all duration-300"
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navItems.map((item, index) => (
              <React.Fragment key={item.name}>
                <a
                  href={item.href}
                  className={`relative text-slate-300 hover:text-purple-500 transition-colors duration-300 py-2 group ${
          ...
          <nav className="flex flex-col p-4">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className={`py-3 text-slate-300 hover:text-purple-500 transition-colors duration-300 ${
                  activeSection === item.href.substring(1) ? 'text-purple-500' : ''
                }`}
